...
def sanitize_catalog(seed):
    ...
    def clean_movie(x):
        return {
            ...
            # The backup has no film artwork; tools/fetch-movie-posters.py fills
            # this in afterwards and main() carries it across regenerations.
            "cachedPoster": x.get("cachedPoster"),
...
def carry_movie_posters(seed):
    """Keep film artwork that fetch-movie-posters.py resolved on an earlier run.

    Those covers come from TMDB, not the backup, so a plain rebuild would drop
    every one of them and send the movie grid back to initial tiles until the
    fetch script ran again. Matched by uuid against whatever is already shipped.
    """
    if not os.path.exists(CATALOG_OUT):
        return 0
    try:
        with open(CATALOG_OUT, encoding="utf-8") as fh:
            known = {
                m["uuid"]: m["cachedPoster"]
                for m in json.load(fh).get("movies", [])
                if m.get("uuid") and m.get("cachedPoster")
            }
    except (json.JSONDecodeError, OSError):
        return 0
    carried = 0
    for movie in seed["movies"]:
        if not movie.get("cachedPoster") and known.get(movie["uuid"]):
            movie["cachedPoster"] = known[movie["uuid"]]
            carried += 1
    return carried
...
def main():
    seed = build()
    carried = carry_movie_posters(seed)
    ...
    movies_with_poster = sum(1 for x in s["movies"] if x.get("cachedPoster"))
    print(
        f"  movies       : {len(s['movies'])}  (cached poster: {movies_with_poster}"
