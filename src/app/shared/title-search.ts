    ...
    @if (query().trim().length >= 2) {
      <section class="ts">
        <header class="ts-head">
          ...
          @if (searching()) {
            <span class="ts-status">Searching TMDB…</span>
          }
        </header>
...
export class TitleSearch {
  ...
  /**
   * Add straight from the result card, and stay put.
   *
   * This used to navigate to the new detail page, which meant adding three
   * films from one search was three round trips back here. The card flips to
   * "✓ In library" on its own — `inLibrary` reads the store — so the result of
   * the click is visible without leaving the search.
   */
  async add(r: TmdbSearchResult): Promise<void> {
    ...
    try {
      const kind = this.kind();
      if (kind === 'show') await this.store.addShow(r);
      else await this.store.addMovie(r);
    } catch (e: any) {
