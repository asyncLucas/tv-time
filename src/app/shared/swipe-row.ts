 ...
 * it a `label2` to enable it): the home rail swipes left to mark watched and
 * right to pause the show. It fires `confirm2` and gets its own hover button.
 *
  ...
  template: `
    <div class="swipe">
      ...
      @if (revealSecondary()) {
        <div
          class="swipe-action"
          [class.right]="direction() === 'left'"
          [class.good]="tone2() === 'good'"
          [class.warn]="tone2() === 'warn'"
          aria-hidden="true"
        >
        ...
        @if (!disabled()) {
          ...
          @if (hasSecondary()) {
            <button
              ...
              [attr.aria-label]="buttonLabel2() || label2()"
            >
              {{ icon2() }}
            </button>
          }
...
export class SwipeRow {
  ...
  /**
   * Open the row from the keyboard — unless the key belongs to something else.
   *
   * The row is a `role="button"` div, so Enter and Space have to open it; but
   * both the swipe actions and anything the caller projects into the row (the
   * episode line on the home rail) can be real controls, and a keydown on one of
   * those bubbles to here. Without this the row's own handler fires *as well* —
   * so tabbing to "About S1·E2" and pressing Enter opened the episode dialog and
   * navigated to the show underneath it. A click doesn't have this problem: the
   * nested buttons stop it (see `onAction`), and stopping a click never stops
   * the keydown that produced it.
   */
  onKey(e: Event): void {
