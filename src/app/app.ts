...
export class App {
  ...
  /** The last TMDB-profile mirror outcome, shown as a dismissible toast. */
  protected readonly tmdbNote = this.store.tmdbNote;
  ...
  constructor() {
    // A mirror note reports on something that already happened elsewhere, so it
    // takes itself away again rather than waiting to be dismissed.
    effect((onCleanup) => {
      if (!this.tmdbNote()) return;
      const timer = setTimeout(() => this.tmdbNote.set(null), TMDB_NOTE_MS);
...
/** How long a TMDB mirror note stays up before clearing itself. */
const TMDB_NOTE_MS = 6000;
