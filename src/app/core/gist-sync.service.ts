...
export type GistStatus = 'off' | 'syncing' | 'synced' | 'offline' | 'error';
...
export class GistSyncService {
  ...
  /**
   * The periodic remote check. A pull that succeeds proves the gist is there
   * and merged, so it also *clears* a stale error state — without this, one
   * transient failure would leave the badge saying "sync error" indefinitely,
   * reading as lost data while everything is in fact stored. `pendingPush`
   * guards the claim: with an edit still queued we are reachable, not synced.
   */
  private async poll(): Promise<void> {
    if (this.busy) return;
    try {
      await this.pull();
      if (!this.busy && !this.pendingPush()) {
        this.status.set('synced');
        this.error.set(null);
        this.lastSync.set(new Date().toISOString());
      }
    } catch {
      // A lone failed GET can be a blip; the next poll or edit retries. But a
      // dead connection is worth naming — "offline" reads as "saved locally,
      // will catch up", where "sync error" reads as "something is wrong".
  ...
  private fail(e: any): void {
    // Failing while offline isn't an error condition — the edit is durable
    // locally and queued for delivery. Say that, instead of "sync error".
    if (!navigator.onLine) {
      this.error.set(null);
