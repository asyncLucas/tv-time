...
export class DeviceService {
  ...
  async init(): Promise<void> {
    ...
    this.bindRoster();
    if (this._signedOut()) {
      // Signed out, yet still holding a gist token: a sign-out always wipes
      // the token (see signOutSelf), so this state can only mean the user
      // reconnected cloud sync after the sign-out. The token IS the
      // credential — honor the reconnect instead of playing dead, which left
      // the badge on "local only" and the settings input empty every launch.
      if (!this.config.get<string>('gistToken')) return;
