...
/**
 * Default signaling server: this project's own Cloudflare Worker (see
 * `signaling/` in the repo — a y-webrtc-protocol relay on Durable Objects).
 *
 * The old y-webrtc defaults (signaling.yjs.dev, the Heroku hosts) are dead and
 * public community relays come and go, so the fleet points at a server we
 * control. It brokers the encrypted handshake only — never your data — and can
 * still be overridden in Settings.
 */
export const DEFAULT_SIGNALING_URL = 'wss://tv-time.lucasluizss.workers.dev';
 ...
 * point — you can pick which one, but not "none". The default below is this
 * project's own Cloudflare Worker; override it in Settings, self-host a relay,
 * or fall back to export/import for a truly zero-dependency path.
 */
...
export class SyncService {
  ...
  /**
   * Presence published by every device in the room, including this one — the
   * live half of the "active sessions" list (DeviceService pairs it with the
   * synced roster). It rides WebRTC *awareness*, which is deliberately not part
   * of the CRDT: it evaporates the moment a device drops off, which is exactly
   * the semantics "currently online" needs.
   */
  readonly presences = signal<Record<string, any>[]>([]);
  /** What this device publishes; re-applied on every (re)connect. */
  private presence: Record<string, any> | null = null;
   ...
   */
  autoStart(): void {
    // A device that was signed out (here or remotely, see DeviceService) keeps
    // its data but stops syncing until it is linked again.
  ...
  /**
   * Describe this device to the rest of the room (id, name, platform). Held on
   * the service so a later connect() — or a reconnect after the signaling
   * server blips — republishes it without the caller having to notice.
   */
  setPresence(presence: Record<string, any> | null): void {
    this.presence = presence;
    this.applyPresence();
  }

  private applyPresence(): void {
    this.provider?.awareness.setLocalStateField('device', this.presence);
  }

  private onAwareness = (): void => {
    const states = this.provider?.awareness.getStates();
    if (!states) return this.presences.set([]);
    this.presences.set(
      [...states.values()].map((s) => s?.['device']).filter((d): d is Record<string, any> => !!d),
    );
  };
