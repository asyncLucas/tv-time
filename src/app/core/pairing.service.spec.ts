...
describe('decodeLink', () => {
  ...
  it('keeps the direction flag, which decides who waits for whom', () => {
    const fromNewDevice = { ...VALID, o: 1 as const };
    expect(decodeLink(code(fromNewDevice))).toEqual(fromNewDevice);
    // A code minted by a signed-in device carries no flag at all.
    expect(decodeLink(code(VALID))?.o).toBeUndefined();
  });
