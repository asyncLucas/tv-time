  ...
  template: `
    <div class="page">
                ...
                @switch (gist.status()) {
                  ...
                  @case ('offline') {
                    Offline — saved on this device{{
                      gistLastSync() ? ', last synced ' + gistLastSync() : ''
                    }}
                  }
      ...
      <section class="card">
        ...
        <h3 class="sub">Your TMDB profile</h3>
        <p class="hint">
          Scores you give episodes, shows and films are sent to TMDB as well as kept here. Without a
          linked account they go anonymously through a temporary guest session — linking one files
          them under your own profile instead, and the link travels with your other synced settings.
        </p>
        <p class="hint">
          With an account linked, your library mirrors there too: adding a title puts it on your TMDB
          watchlist, favorites follow your ★, and removing a title takes it back off. It is one-way —
          your library here is always the record, and a change TMDB refuses never undoes it.
        </p>
        @if (tmdb.hasAccount()) {
          <div class="synced">
        ...
        <h3 class="sub">Your custom lists</h3>
        <p class="hint">
          Lists are the one thing the link above can't carry. TMDB's older API can only put
          <em>films</em> on a list, and yours hold shows too, so lists go through its v4 API — which
          needs the second credential on that same TMDB page (the long
          <em>API Read Access Token</em>) and its own one-time approval. Everything else keeps
          working whether or not you set this up.
        </p>
        <div class="row">
          <input
            class="in"
            type="password"
            placeholder="TMDB v4 API read access token"
            [value]="v4Token()"
            (input)="v4Token.set($any($event.target).value)"
          />
          <button class="btn primary" (click)="saveV4Token()">Save</button>
        </div>
        @if (tmdb.hasListAccess()) {
          <div class="synced">
            <div class="status ok">✓ Lists linked</div>
            <button class="btn" [disabled]="listBusy()" (click)="syncLists()">
              {{ listBusy() ? 'Syncing…' : 'Sync my lists now' }}
            </button>
            <button class="btn" (click)="unlinkLists()">Unlink</button>
          </div>
          <p class="hint" style="margin-top:10px">
            New lists appear on TMDB as soon as you put something on them; renames, removals and
            deletions follow. They are created <strong>private</strong> — publishing them is yours to
            decide, on TMDB. Use <em>Sync my lists now</em> once to send the lists you already had.
          </p>
        } @else if (listLink(); as link) {
          <div class="row">
            <a class="btn primary" [href]="link.approveUrl" target="_blank" rel="noopener">
              Approve on TMDB ↗
            </a>
            <button class="btn" [disabled]="listBusy()" (click)="finishListLink()">
              {{ listBusy() ? 'Checking…' : "I've approved it — finish" }}
            </button>
            <button class="btn ghost" (click)="listLink.set(null)">Cancel</button>
          </div>
          <p class="hint" style="margin-top:10px">
            Approve the request in the tab that opens, then come back and finish. This is a separate
            approval from the account link above — TMDB treats them as different permissions.
          </p>
        } @else {
          <div class="row">
            <button
              class="btn"
              [disabled]="!tmdb.hasV4Token() || listBusy()"
              (click)="startListLink()"
            >
              {{ listBusy() ? 'Starting…' : 'Link lists' }}
            </button>
          </div>
          @if (!tmdb.hasV4Token()) {
            <p class="hint" style="margin-top:10px">Save a v4 read access token above to enable this.</p>
          }
        }
        @if (listErr()) { <p class="hint err-hint">{{ listErr() }}</p> }
        @if (listMsg()) { <p class="hint">{{ listMsg() }}</p> }
      </section>
      ...
      <div class="about">
        <a href="https://github.com/asyncLucas/tv-time" target="_blank" rel="noopener">
          GitHub ↗
        </a>
        <span>v{{ build.version }} · built {{ builtAt }}</span>
      </div>
    </div>
  `,
  styles: [
...
export class Settings {
  ...
  /** Stamped by tools/stamp-build.mjs on every `npm run build` (see prebuild). */
  readonly build = buildInfo;
  readonly builtAt = new Date(buildInfo.builtAt).toLocaleDateString(undefined, {
    year: 'numeric',
    month: 'short',
    day: 'numeric',
  });
  ...
  /** The v4 half — a separate credential, a separate approval (see the card). */
  v4Token = signal('');
  listLink = signal<{ requestToken: string; approveUrl: string } | null>(null);
  listErr = signal('');
  listMsg = signal('');
  ...
  saveV4Token(): void {
    this.tmdb.setV4Token(this.v4Token());
    this.v4Token.set('');
    this.listErr.set('');
    this.listMsg.set('Read access token saved.');
  }

  /** The v4 flow, step one. Same shape as the account link, different endpoint. */
  async startListLink(): Promise<void> {
    this.listErr.set('');
    this.listMsg.set('');
    this.listBusy.set(true);
    try {
      this.listLink.set(await this.tmdb.startListLink());
    } catch (e: any) {
      this.listErr.set(String(e?.message ?? e));
    } finally {
      this.listBusy.set(false);
    }
  }

  async finishListLink(): Promise<void> {
    const link = this.listLink();
    if (!link) return;
    this.listErr.set('');
    this.listBusy.set(true);
    try {
      await this.tmdb.finishListLink(link.requestToken);
      this.listLink.set(null);
      this.listMsg.set('Lists linked. Use “Sync my lists now” to send the ones you already have.');
    } catch (e: any) {
      this.listErr.set(String(e?.message ?? e));
    } finally {
      this.listBusy.set(false);
  ...
  async unlinkLists(): Promise<void> {
    this.listErr.set('');
    this.listMsg.set('');
    await this.tmdb.unlinkLists();
  }

  /** The one-off catch-up for lists that predate linking. */
  async syncLists(): Promise<void> {
    this.listErr.set('');
    this.listMsg.set('');
    this.listBusy.set(true);
    try {
      const { lists, items, unresolved } = await this.store.syncAllLists();
      this.listMsg.set(
        `Sent ${items} title(s) across ${lists} list(s).` +
          // A title TMDB has no id for isn't a failure worth alarming over, but
          // silently sending fewer than the list holds would look like one.
          (unresolved ? ` ${unresolved} couldn't be matched to a TMDB title.` : ''),
      );
    } catch (e: any) {
      this.listErr.set(String(e?.message ?? e));
    } finally {
  ...
  async connectGist(): Promise<void> {
    const t = this.gistToken().trim();
    this.gistToken.set('');
    if (!t) return;
    await this.gist.connect(t);
    // Pasting a working token is a re-authorization: without this, a device
    // that was once signed out kept its `signedOut` flag, so every later
    // launch skipped autoStart and the UI read as never connected.
    if (this.gist.status() !== 'error') await this.devices.reactivate();
  }
