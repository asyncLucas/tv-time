        ...
        <div>
          ...
          <div class="sub">
            @if (store.lists().length) {
              {{ store.lists().length }} {{ store.lists().length === 1 ? 'list' : 'lists' }} ·
          ...
          @for (list of store.lists(); track list.id) {
            <section class="list">
              <header>
                <div class="l-head">
                  <h2>{{ list.name }}</h2>
                  <span class="count">{{ list.items?.length || 0 }}</span>
                </div>
                <div class="l-actions">
              ...
              @if (list.items?.length) {
                    ...
                    <app-swipe-row (open)="openItem(item)" (remove)="store.removeListItem(list.id, item)">
                      @if (store.resolveListItem(item); as r) {
                        ...
                        <div class="item unmatched">
                          <div class="thumb ph"><span>{{ item.title | initials }}</span></div>
                          <div class="it-main">
                ...
                @if ((list.items?.length || 0) > PREVIEW) {
                  <button class="more" (click)="toggle(list.id)">
                    {{ expanded().has(list.id) ? 'Show less' : 'Show all ' + list.items.length }}
                  </button>
                }
              } @else {
...
export class Lists {
  ...
  /** The rows a card renders: a short preview until the user expands it. */
  visible(list: { id: string; items?: ListItem[] }): ListItem[] {
    const items = list.items ?? [];
    return this.expanded().has(list.id) ? items : items.slice(0, PREVIEW);
  }
